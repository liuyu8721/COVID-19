...
table(dynamicMods)
group.row.id = which(dynamicMods %in% c(1)) # Take GRY/B.1.1.7 for exapmle
group.adj <- adjacency[group.row.id, group.row.id]
pheatmap(group.adj, cluster_rows = F, cluster_cols = F)
diag(group.adj) <- 0
degree.cutoff <- 0.9 # Cutoff for intra-modular core mutations 
...
# Variant definition using core mutations
group.adj.high.id
group.core.info <- word(word(group.adj.high.id, 2, 2, fixed(":")), 1, 1, fixed(","))

load("D:/00SARS-Cov-2/GibHub/data/all.count.RData")
load("D:/00SARS-Cov-2/GibHub/data/all.muta.RData")

all.muta.small <- all.muta %>%
  mutate(mutation = paste0(refvar, refpos, qvar)) %>%
  select(sample, mutation)

# Genomes with core mutations
group.sample <- c()
group.nsample <- c()
for (m in group.core.info) {
  if (m==group.core.info[1]) {
    group.sample <- all.muta.small %>%
      filter(mutation==m) %>%
      pull(sample)
  } else {
...
plot(group.nsample)
data.frame(n = group.nsample,
           percent = group.nsample / nrow(all.count))

# NextStrain or Pangolin classification for identified genomes
group.clade <- all.count %>%
  filter(Accession_ID %in% group.sample) %>%
  select(Accession_ID, clade, lineage, country, Collection_week)
data.frame(table(group.clade$clade))
data.frame(table(group.clade$lineage))
