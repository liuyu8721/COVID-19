...
fpath="Folder_with_MUMmer_Output_SNPs_and_INDELs_Files"
setwd(fpath)
flist <- list.files()
flist <- flist[grep("snps", flist)]
...
for (f in flist) {
  ...
  fl <- file(f, open="rb")
  nlines <- 0L
  while (length(chunk <- readBin(fl, "raw", 65536)) > 0) {
  ...
  if (nlines > 4) {
    x <- read.delim(f, skip = 4, header = FALSE, stringsAsFactors = FALSE, colClasses = "character")
    colnames(x) <- c("rpos","rvar","qvar","qpos","","","","","rlength","qlength","","","rname","qname")
    # x$qname <- word(f, 1, 1, fixed("."))
      
    nucmer <- rbind(nucmer, x)
  }
}
nucmer$rpos <- as.numeric(nucmer$rpos)
nucmer$qpos <- as.numeric(nucmer$qpos)

###################################################################################################

prefix <- "Output_Path_and_File_Name"
...
# Fix IUPAC codes
table(nucmer$qvar)
nucmer <- nucmer[!nucmer$qvar%in%c("B","D","H","K","M","N","R","S","V","W","Y", "U"),]
nrow(nucmer)

### Aminoacid variant list ----
# Load reference sequence
refseq<-read.fasta("NC_045512.2.fa",forceDNAtolower=FALSE)[[1]] #Download from NCBI

# Load GFF3
gff3<-read.delim("NC_045512.2_annot.gff3",as.is=TRUE,skip=2,header=FALSE) #Download from NCBI
...
results$sample <- word(results$sample, 2, 2, fixed("|"))
write.csv(results, file = paste0(prefix, "_annot.csv"), row.names = FALSE)
